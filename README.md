...
epubveri is **pre-1.0 and not yet published** to crates.io or npm — it's
still under active development. This is not a drop-in replacement for
epubcheck yet, and this project is deliberately upfront about that
...
**Using it as a library** (inside your own Rust project): add it as a
path or git dependency (it isn't published yet — see "How good is it
right now?" above), then:
