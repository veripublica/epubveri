...
upfront about that rather than overclaiming. (It ships as the Rust crate
[`epubveri`](https://crates.io/crates/epubveri) on crates.io and a
WebAssembly build on npm as
[`@veripublica/epubveri-wasm`](https://www.npmjs.com/package/@veripublica/epubveri-wasm),
both released together and published from CI with a provenance
attestation. See [`CHANGELOG.md`](./CHANGELOG.md) for what's changed
between releases.)
...
- **99.7%** of the test suite's "this should be flagged" cases are
  ...
  would report (685 of 687). The two that are not are one rule epubveri
  deliberately does not implement: which elements may carry which ARIA
  role, which needs epubcheck's own per-element table rather than a guess.
...
such a fixture into a minimal book. **Four** fixtures are written for that
loose-file mode in a way the wrap cannot reproduce — they are about a
...
critical), so epubveri is the one that is right on those books. The other two
are an artefact of epubcheck's own error recovery and a documented, deliberate
...
the fields (`rule`, `element_path`, `violation_kind`, `advisory_basis`) that
exist for machines and never appear in the human report, how to show findings
to a person without making a valid book look broken, which release archive to
pick and how to verify it, and how often you should actually update. It ends
with a small worked example you can run. The envelope itself is specified
...
**Using epubveri, or building something that calls it?
Read [`LICENSING.md`](./LICENSING.md).** Short version: the books you check are
yours unconditionally, checking them commercially needs no commercial license,
and `epubveri` never writes to a book at all — it reads and reports, so nothing
of it can end up inside an EPUB. The commercial license is for embedding this
code in a closed-source product you distribute, or serving a modified version
over a network. Nothing else.

`LICENSING.md` links onward to a single
[licensing FAQ](https://github.com/veripublica/epubsana/blob/main/LICENSING-FAQ.md)
shared with the sibling project `epubsana`, which carries the
GPL-compatibility reasoning. That text is deliberately kept in one place so the
two projects cannot drift into saying different things. `LICENSE` governs
anywhere any of them could be read differently.
...
- **Building a plugin or a tool on epubveri** — what to parse, what is stable,
  what only machines can see, how to present findings, how to verify the binary
  you downloaded, and how often to update; with a worked example —
